---
name: troubleshoot-external-logging
description: Use when assessing or diagnosing a support ticket involving Qubership External Logging Installer (External Logging Installer) or its components (Graylog, OpenSearch, MongoDB, nginx, Keepalived, the FluentD or FluentBit agent on the Graylog VM, or the Ansible install run), including installation, configuration, and runtime failures. Read-only and advisory; no live system access.
---
...
Diagnose one reported problem with the External Logging Installer or the stack it deploys. Read the user's description
and any attached evidence, match it to a case in the reference, and return a diagnosis the operator can act on. This is
the whole skill — there is no classification step and no ticket handling.
...
2. **Localize.** From the text and the log signatures, name the **component** (`graylog` / `opensearch` / `mongodb` /
   `nginx` / `keepalived` / `fluentd` / `fluentbit` / `ansible`).
